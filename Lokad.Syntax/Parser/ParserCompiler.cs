                ...
                else
                {
                    ...
                    for (var i = reduced.Provided.Count - 1; i >= 0; --i)
                    {
                        ...
                        if (!reduced.Provided[i])
                        {
                            ...
                            {
                                args[i] = Expression.NewArrayBounds(t.GetElementType(), Expression.Constant(0));
                            }
                        ...
                        else if (t.IsArray)
                        {
                            ...
                            body.Add(Expression.Assign(arg,
                                Expression.NewArrayBounds(t.GetElementType(), len)));

                            // do { arg[--len] = stack.Pop() } while (len > 0)
                            var stack = _stacks[t.GetElementType()];
